...
####################################################################
## Sarkar Supplementary Figure S2 
...
## Two genes with one missing background each ... 
## missing in: 40139 39875
## missing in: 40139 39875
...
Figure \ref{fig:intensityPlots}, rows two through six, plots the
...
####################################################################
## 1. Evaluation using Sarkar density plots 
...
####################################################################
## 3. Spread (MAD) vs. location (median) of all probes, highlight spike-ins
##    Figure 4 in Sarkar ...
####################################################################
...
> 0.95$), we decided to use only those two replicates from each day for
...
(package \pkg{microRNA}).  The mouse specific miRNA names are first extracted, then 
converted to the standard nomenclature:
...
ids1.names <- avedata$genes$Name[which(avedata$genes[,"Gene ID"]%in%ids1)]
ids1.mmu.names <- lapply(strsplit(ids1.names, " "), function(x) grep("MMU", x, value=TRUE))
ids1.mmu.names <- unlist(lapply(ids1.mmu.names, function(x) gsub("\\;", "", x)))

## Convert to standard nomenclature ... 
miRs1 <- tolower(ids1.mmu.names)
miRs1 <- gsub("mir", "miR", miRs1) 
...
if (require("microRNA")) {
  ...
  length(targets.miRB) ## 13126, yikes!!! 
  ## target ensembl IDs
...
\pkg{org.Mm.eg.db} Bioconductor package.  The final list contains 1,951
...
if (require("org.Mm.eg.db")) {
  ...
  length(targets.intsect)  ## Only 1951
}
...
Next, these Entrez Gene IDs are reverse mapped back to the set of
...
four of the fifteen identified miRNAs. 
